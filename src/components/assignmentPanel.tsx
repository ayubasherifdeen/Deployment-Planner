  ...
  return (
    <div className="border-t border-indigo-200 bg-indigo-50 p-4">
      ...
      <div className="flex flex-col gap-2">
        {eligible.map(person => {
  const isUnavailable = person.assignedMissionIds.length >= MAX_MISSIONS
      ...
      <div className="flex flex-col gap-1.5 border-t border-gray-100 bg-gray-50 px-4 py-2">
        ...
        {person.matched.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {person.matched.map(skill => (
              <span key={skill} className="rounded-md bg-green-100 px-1.5 py-0.5 text-xs font-semibold text-green-700">
                ✓ {skill}
              </span>
            ))}
          </div>
        ...
        {person.missing.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {person.missing.map(skill => (
              <span key={skill} className="rounded-md bg-red-100 px-1.5 py-0.5 text-xs font-semibold text-red-700">
                ✗ {skill}
              </span>
            ))}
          </div>
        ...
        <div className="flex items-center justify-between pt-1">
          <span className={`rounded-md px-2 py-0.5 text-xs font-bold
            ${person.score === 100 ? "bg-green-100 text-green-700"
            : person.score >= 50  ? "bg-yellow-100 text-yellow-700"
          ...
          {isUnavailable ? (
            ...
            >
              {isChecked && (
                <svg className="h-2.5 w-2.5" fill="none" stroke="white" strokeWidth={3} viewBox="0 0 12 12">
