...
export function WarningPanel({ persons }: WarningPanelProps) {
  // overstreched = assigned to 3 or more missions
  const overstreched = persons.filter(p => p.assignedMissionIds.length >= 3);

  if (overstreched.length === 0) {
    return (
      ...
      {overstreched.map(person => {
        ...
        return (
          <div key={person.id} className="flex items-start gap-3 rounded-xl border border-red-200 bg-red-50 p-3">
            ...
            <div>
              <p className="text-sm font-semibold text-red-700">{person.name} is overstretched</p>
